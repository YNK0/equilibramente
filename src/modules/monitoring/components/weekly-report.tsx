...
export function WeeklyReport({ week, prevWeek }: Props) {
  ...
  const comparison = prevWeek ? {
    ...
    regulation: (summary.total_regulation_minutes ?? 0) - (prevWeek.summary?.total_regulation_minutes ?? 0),
  } : null;
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-4 bg-white rounded-xl border border-gray-100 text-center">
          <span className="text-2xl font-bold text-purple-700">{summary.total_tasks_completed}</span>
          <p className="text-xs text-gray-400 mt-0.5">Tareas completadas</p>
          {comparison && comparison.tasks !== 0 && (
            <span className={`text-[10px] ${comparison.tasks > 0 ? 'text-green-500' : 'text-red-500'}`}>
              {comparison.tasks > 0 ? '+' : ''}{comparison.tasks} vs semana anterior
            </span>
          )}
        </div>
        <div className="p-4 bg-white rounded-xl border border-gray-100 text-center">
          ...
          {comparison && comparison.mood !== 0 && (
            <span className={`text-[10px] ${comparison.mood > 0 ? 'text-green-500' : 'text-red-500'}`}>
              {comparison.mood > 0 ? '+' : ''}{comparison.mood} vs semana anterior
            </span>
          )}
        </div>
      </div>
      ...
      {summary.achievements_unlocked > 0 && (
        <div className="p-4 bg-amber-50 rounded-xl border border-amber-200 text-center">
          <p className="text-sm font-semibold text-amber-800">
            🏆 {summary.achievements_unlocked} logro{summary.achievements_unlocked > 1 ? 's' : ''} desbloqueado{summary.achievements_unlocked > 1 ? 's' : ''} esta semana
          </p>
        </div>
      )}
    </div>
